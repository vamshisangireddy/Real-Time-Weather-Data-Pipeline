...
| (WeatherAPI)   |       | (weather_producer.py) |    | (Docker)        |
+----------------+       +-------------------+       +--------^--------+
                                                              |  weather_raw_data
                                                              |  topic
+----------------+       +-------------------------+       +-----------------+
| AWS Glue Data  |<------| PySpark Streaming App   |<------|  Apache Kafka   |
| Catalog        |       | (spark_iceberg_stream.py)|      | (Docker)        |
+--------^-------+       +-------------------------+       +--------^--------+
         |                                                       |
         |                                                       |
         |         +-------------------------------------+       |
...
* **Python 3.7+**
* **confluent-kafka-python, requests, python-dotenv**
...
1. Create and navigate to your project directory:
   ...
   ```
2. Create a `docker-compose.yml` including ZooKeeper and Kafka services.
3. Start the cluster:
...
2. **IAM Policy**: Create and attach permissions for S3, Glue, and DynamoDB locks.
...
3. **Spark Streaming**:
   ...
   /path/to/spark/bin/spark-submit \
     ...
     --conf spark.sql.catalog.glue_catalog.warehouse="s3://your-pyspark-weather-iceberg-lake-123/warehouse" \
     --conf spark.sql.catalog.glue_catalog.io-impl=org.apache.iceberg.aws.s3.S3FileIO \
     --conf spark.sql.catalog.glue_catalog.lock-impl=org.apache.iceberg.aws.glue.DynamoDbLockManager \
     --conf spark.sql.catalog.glue_catalog.region="us-west-2" \
     --conf spark.sql.extensions=org.apache.iceberg.spark.extensions.IcebergSparkSessionExtensions \
     --conf spark.sql.defaultCatalog=glue_catalog \
     spark_iceberg_weather_stream.py
   ```
